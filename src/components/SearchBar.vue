<template>
  ...
  <Modal :open-modal="openModal" :set-close-listener="false" @closemodal="close">
    <div class="w-form">
      <!-- Search Input -->
      <div class="p-1">
        <input ref="input" v-model="inputValue" type="search" autocomplete="off" spellcheck="false"
          :placeholder="t`Type to search...`" class="
            ...
            rounded-md
            p-3
          " @keydown.up="up" @keydown.down="down" @keydown.enter="() => select()" @keydown.esc="close" />
      </div>
      ...
      >
        ...
        >
          ...
          <div class="flex w-full justify-between px-3 items-center"
            ...
            <p class="text-sm text-end justify-self-end" :class="`text-${groupColorMap[si.group]}-500`">
              {{
      ...
      <div class="m-1 flex justify-between flex-col gap-2 text-sm select-none">
        ...
        <div class="flex justify-between">
          <div class="flex gap-1">
            <button v-for="g in searchGroups" :key="g" class="border px-1 py-0.5 rounded-lg"
              :class="getGroupFilterButtonClass(g)" @click="searcher!.set(g, !searcher!.filters.groupFilters[g])">
              {{ groupLabelMap[g] }}
            </button>
          </div>
          <button class="hover:text-gray-900 py-0.5 rounded text-gray-700" @click="showMore = !showMore">
        ...
        <div v-if="showMore" class="-mt-1">
          ...
          <div class="flex gap-1 text-gray-800">
            <button v-for="s in ['skipTables', 'skipTransactions'] as const" :key="s"
              class="border px-1 py-0.5 rounded-lg" :class="{ 'bg-gray-200': searcher?.filters[s] }"
          ...
          <div class="flex mt-1 gap-1 text-blue-500 flex-wrap">
            <button v-for="sf in schemaFilters" :key="sf.value" class="
                border
                px-1
                py-0.5
                rounded-lg
  ...
  ">
        ...
        <div class="flex text-sm text-gray-500 justify-between items-baseline">
          ...
          <div v-if="(searcher?.numSearches ?? 0) > 50" class="border border-gray-100 rounded flex justify-self-end ms-2">
            <template v-for="c in allowedLimits.filter(
              (c) => c < (searcher?.numSearches ?? 0) || c === -1
            )" :key="c + '-count'">
              <button class="w-9" :class="limit === c ? 'bg-gray-100' : ''" @click="limit = Number(c)">
                {{ c === -1 ? t`All` : c }}
              </button>
            </template>
          </div>
...
import { searcherKey, shortcutsKey, languageDirectionKey } from 'src/utils/injectionKeys';
